...
void main()
{
    ...
    float fogWeight = (1.0/exp(fogConstant*length(vec3(pow(sqrt(dir.x * dir.x + dir.z * dir.z)/sqrt(2), 3.0) * 60.0, sqrt(max(0, dir.y)) * 2.5, pow(sqrt(dir.x * dir.x + dir.z * dir.z)/sqrt(2), 3.0) * 60.0))));

	vec4 result = texture(skybox, TexCoords);
    if(dir.y < 0){
        result.xyz = fogColor;
        fogWeight = 1.0/exp(fogConstant*length(vec3(pow(sqrt(dir.x * dir.x + dir.z * dir.z)/sqrt(2), 50.0) * 50.0, sqrt(max(0, -dir.y)) * 2.5, pow(sqrt(dir.x * dir.x + dir.z * dir.z)/sqrt(2), 50.0) * 50.0)));
